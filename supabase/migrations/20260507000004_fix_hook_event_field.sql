-- Fix: Supabase Auth sends the user ID as 'user_id' (snake_case), not 'userId'.
-- The hook was reading event->>'userId' which always returned null, so tenant_id
-- was never injected and all RLS queries returned empty results.
