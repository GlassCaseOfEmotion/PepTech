...
import {
  SparklesIcon as SparklesO, InboxIcon as InboxO, UsersIcon as UsersO,
  CubeIcon as CubeO, WalletIcon as WalletO, BeakerIcon as BeakerO,
  PaperAirplaneIcon as PaperAirplaneO, BoltIcon as BoltO,
  ArchiveBoxIcon as ArchiveBoxO, PhotoIcon as PhotoO, Cog6ToothIcon as Cog6ToothO,
} from '@heroicons/react/24/outline'
import {
  SparklesIcon as SparklesS, InboxIcon as InboxS, UsersIcon as UsersS,
  CubeIcon as CubeS, WalletIcon as WalletS, BeakerIcon as BeakerS,
  PaperAirplaneIcon as PaperAirplaneS, BoltIcon as BoltS,
  ArchiveBoxIcon as ArchiveBoxS, PhotoIcon as PhotoS, Cog6ToothIcon as Cog6ToothS,
...
interface IconProps {
  ...
  /** Render the filled variant if the icon has one (Heroicons-backed nav icons).
   * Silently ignored by outline-only PtIcon glyphs. */
  solid?: boolean
}
...
type HeroIcon = React.ForwardRefExoticComponent<
  Omit<React.SVGProps<SVGSVGElement>, 'ref'> & { title?: string; titleId?: string } & React.RefAttributes<SVGSVGElement>
>

/** Build an Icons.* component backed by paired Heroicons (outline + solid).
 * Same call surface as PtIcon-based glyphs: {size, className, style} — plus a
 * `solid` opt-in that swaps to the filled variant. */
function makeIcon(Outline: HeroIcon, Solid: HeroIcon) {
  return function HeroBacked({ size = 16, solid = false, className, style }: IconProps) {
    const Cmp = solid ? Solid : Outline
    return <Cmp width={size} height={size} className={className} style={{ flexShrink: 0, ...style }} aria-hidden />
  }
}

export const Icons = {
  // Nav icons — Heroicons-backed, support `solid` prop for active state
  inbox:   makeIcon(InboxO, InboxS),
  users:   makeIcon(UsersO, UsersS),
  box:     makeIcon(CubeO, CubeS),
  flask:   makeIcon(BeakerO, BeakerS),
  send:    makeIcon(PaperAirplaneO, PaperAirplaneS),
  zap:     makeIcon(BoltO, BoltS),
  vault:   makeIcon(ArchiveBoxO, ArchiveBoxS),
  ...
  spark:   makeIcon(SparklesO, SparklesS),
  gear:    makeIcon(Cog6ToothO, Cog6ToothS),
  ...
  wallet:  makeIcon(WalletO, WalletS),
